    ...
    return (
        <div className="bg-neutral-900">
            <div className="container mx-auto px-4 md:px-6 lg:px-8">
                <div className="flex items-center justify-between gap-4 md:gap-6 py-2 text-xs font-medium">
                    ...
                    <div className="relative">
                        ...
                        {isRegionsOpen && (
                            <div
                                className="absolute left-0 top-full mt-2 w-56 bg-white border border-neutral-100 rounded-lg shadow-lg py-2 z-50"
                                onMouseEnter={() => setIsRegionsOpen(true)}
                                onMouseLeave={() => setIsRegionsOpen(false)}
                            >
                                {regions.map((region) => (
                                    <Link
                                        key={region.href}
                                        href={region.href}
                                        className="block px-4 py-2 text-sm text-slate-700 hover:bg-brand-50 hover:text-brand-700 transition-colors"
                                    >
                                        {region.name}
                                    </Link>
                                ))}
